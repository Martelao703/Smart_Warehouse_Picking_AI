...
class Window(tk.Tk):
    ...
    def __init__(self):
        ...
        recombination_methods = ['PMX', 'OX', 'UX']
        ...
        mutation_methods = ['Insert', 'MutationSwap', 'MutationShuffleForkProds']
    ...
    def runGA_button_clicked(self):
        ...
        recombination_method = RecombinationPMX(
            ...
            RecombinationSpx(float(self.entry_recombination_prob.get())) if recombination_methods_index == 1 else \
                RecombinationUx(float(self.entry_recombination_prob.get()))
        ...
        mutation_method = MutationInsert(
            ...
            MutationSwap(float(self.entry_mutation_prob.get())) if mutation_methods_index == 1 else \
                MutationShuffleForkProds(float(self.entry_mutation_prob.get()))
...
class SearchSolver(threading.Thread):
    ...
    def run(self):
        for pair in self.agent.pairs:
            initial_state = self.gui.initial_state
...
class SolutionRunner(threading.Thread):
    ...
    def run(self):
        ...
        products = self.best_in_run.get_sorted_prods()
        ...
        for step in range(steps):
            ...
            for j in range(len(forklift_path)):
                if not forklift_path[j]:
                    continue  # Skip empty forklift paths
                ...
                # PAINT PRODUCTS
                left_cell = Cell(old_cell[j].line, old_cell[j].column - 1)
                right_cell = Cell(old_cell[j].line, old_cell[j].column + 1)

                if len(products[j]) > 0:
                    if self.state.is_valid(left_cell.line, left_cell.column) and left_cell == products[j][0]:
                        self.state.matrix[left_cell.line][left_cell.column] = constants.PRODUCT_CATCH
                        products[j].pop(0)

                    elif self.state.is_valid(right_cell.line, right_cell.column) and right_cell == products[j][0]:
                        self.state.matrix[right_cell.line][right_cell.column] = constants.PRODUCT_CATCH
                        products[j].pop(0)
